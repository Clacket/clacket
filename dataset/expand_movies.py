...
#       If a movie does not have a certain field, it will contain 'N/A' as the
#       value of the field.
                ...
                if index + 1 >= start:
                    try:
                        ...
                        try:
                            ...
                            # if not the movie, or the response isn't
                        ...
                        print('There is an error in {0}\'s format at line {1}:'
                              '\nEvery line should have 3 comma-separated '
                              'fields: id,year_of_release,title'
                              .format(file_path, str(index + 1)))
                        break
                    except (Timeout, ConnectionError):
                        # if request times out or there's a connection error
                        time.sleep(30)
                else:
                    break
                trials += 1
            if trials > 20:
                print('Could not resolve connection issues after 20 trials. '
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-f', '--file',
        help='Path for the file that contains the movie titles '
        '(every line is a movie in the following format: '
        'id,year_of_release,title',
        type=str, default='movie_titles.txt', metavar='')
    parser.add_argument(
        '-en', '--encoding',
        help='Encoding of input file: UTF-8 or ISO-8859-1.',
        type=str, default='ISO-8859-1',
        choices=('UTF-8', 'ISO-8859-1'), metavar='')
    parser.add_argument(
        '-exp', '--oexpanded',
        help='Path for the file where the expanded movies '
        'should be output (does not have to exist).',
    ...
    if Path(args.file).is_file():
        expand(
            args.start, args.file, args.encoding, args.oexpanded, args.ofailed)
    else:
